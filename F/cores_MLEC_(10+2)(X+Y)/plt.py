...
def ReadData():
    ...
    for line in lines[1:]:
        ...
        loc_n = int(loc_n)
        loc_k = int(loc_k)
        if (net_n == 10) and (net_k == 2) and (loc_n <= MAX_LOC_N) and (loc_k <= MAX_LOC_K):
...
def GenerateHeatmap(data):
    ...
    # Set title
    ax.set_title("Number of Cores to Achieve 600 Gbps for MLEC (10+2)/(X+Y)", fontdict={'fontsize': 16}, y=1.08)
