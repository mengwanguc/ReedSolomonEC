...
def ReadData():
    ...
    for line in lines[1:]:
        ...
        loc_n = int(loc_n)
        loc_k = int(loc_k)
        if (loc_n == 10) and (loc_k == 2) and (loc_n <= MAX_NET_N) and (loc_k <= MAX_NET_K):
...
def GenerateHeatmap(data):
    ...
    # Set title
    ax.set_title("Serial MLEC (X+Y)/(10+2) Encoding Throughput Heatmap", fontdict={'fontsize': 16}, y=1.08)
