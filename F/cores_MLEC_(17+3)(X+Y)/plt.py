...
def ReadData():
    ...
    array = [[np.nan for n in range(MAX_LOC_N + 1)] for k in range(MAX_LOC_K + 1)]
    ...
    for line in lines[1:]:
        ...
        loc_n = int(loc_n)
        loc_k = int(loc_k)
        if (net_n == 17) and (net_k == 3) and (loc_n <= MAX_LOC_N) and (loc_k <= MAX_LOC_K):
...
def GenerateHeatmap(data):
    ...
    # X-Y axis labels
    ax.set_ylabel("Local Parity Units K", fontsize=12)
    ax.set_xlabel("Local Data Units N", fontsize=12)
    ax.invert_yaxis()
    ...
    # Set title
    ax.set_title("Number of Cores to Achieve 600 Gbps for MLEC (17+3)/(X+Y)", fontdict={'fontsize': 16}, y=1.08)
