...
def ReadData():
    ...
    array = [[np.nan for n in range(MAX_LRC_K + 1)] for k in range(MAX_LRC_R + 1)]
    ...
    for line in lines[1:]:
        ...
        if (l == 2) and (p == 1) and (k <= MAX_LRC_K) and (r <= MAX_LRC_R):
            ...
            array[r][k] = float(throughput) / 1000
...
def GenerateHeatmap(data):
    ...
    # Set title
    ax.set_title("LRC (X, 2, Y) Encoding Throughput Heatmap", fontdict={'fontsize': 16}, y=1.08)
